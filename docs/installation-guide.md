...
Timeshift - это приложение для создания снимков системы. Если вы используете тему Adwaita:dark и хотите, чтобы Timeshift отображался с тёмной темой, выполните следующие шаги:

1. Скопируйте системный файл запуска:
   ```bash
   cp /usr/share/applications/timeshift-gtk.desktop ~/.local/share/applications/
   ```

   Это создаст пользовательскую версию, которая переопределит оригинал.

2. Отредактируйте пользовательский файл запуска:
   ```bash
   nano ~/.local/share/applications/timeshift-gtk.desktop
   ```

3. Найдите строку `Exec=` и измените её следующим образом:
   ```
   Exec=env GTK_THEME=Adwaita:dark timeshift-launcher
   ```

4. Сохраните файл и обновите кэш приложений:
   ```bash
   gtk-update-icon-cache
   ```

Теперь Timeshift будет запускаться с тёмной темой Adwaita:dark.
